...
- `base.json`：Base、数据表、默认 View、字段标识和 Base 链接；
...
- `abstract_zh` 必填；
- `keywords` 必填，表示论文关键词数组，必须包含 2–4 个合法中文或中英混合技术关键词；
- 飞书“关键词”列是文本字段，脚本将数组元素用中文顿号 `、` 连接，enrichment 中不要自行拼接字符串。
...
论文关键词应优先覆盖研究任务、方法类别、数据或应用领域、关键学习范式。每篇论文生成 2–4 个关键词，优先 2–3 个；只有存在明确且互不重复的补充维度时才使用 4 个。每个关键词最长 6 个字，至少包含一个中文字符；优先使用短中文技术词，允许 `时序`、`时序LLM`、`3D视觉` 这类缩写或中英混合词；不为了凑数生成 `时间序列`、`机器学习` 等过宽泛关键词。关键词不允许纯英文、空值、重复值、完整句子、作者名或论文标题直抄。
...
`daily-highlights.md` 是“今日要点”的首选输入文件。`domain-overview.txt` 也可作为同一内容输入，但语义必须是今日要点而不是泛泛主题概述。
