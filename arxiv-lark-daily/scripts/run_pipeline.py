...
def validate_keywords(keywords: Any, paper: dict[str, Any]) -> list[str]:
    canonical_id = paper.get("canonical_id", "未知论文")
    if not isinstance(keywords, list):
        raise SkillError(f"论文关键词必须是数组: {canonical_id}")
    cleaned = []
    seen = set()
    for value in keywords:
        if not isinstance(value, str):
            raise SkillError(f"论文关键词必须是字符串: {canonical_id}")
        keyword = " ".join(value.strip().split())
        if not keyword:
            raise SkillError(f"论文关键词不能为空: {canonical_id}")
        if keyword in seen:
            raise SkillError(f"论文关键词重复: {canonical_id}: {keyword}")
        if not CHINESE_PATTERN.search(keyword):
            raise SkillError(f"论文关键词必须包含中文字符: {canonical_id}: {keyword}")
        if ASCII_WORD_PATTERN.fullmatch(keyword):
            raise SkillError(f"论文关键词不能是纯英文或纯 ASCII: {canonical_id}: {keyword}")
        if SENTENCE_MARK_PATTERN.search(keyword):
            raise SkillError(f"论文关键词应是简短主题词，不能是句子: {canonical_id}: {keyword}")
        if len(keyword) > 6:
            raise SkillError(f"论文关键词最长不超过 6 个字: {canonical_id}: {keyword}")
        title = str(paper.get("title", "")).strip()
        if title and keyword.lower() == title.lower():
            raise SkillError(f"论文关键词不能直接使用论文标题: {canonical_id}")
        seen.add(keyword)
        cleaned.append(keyword)
    if not 2 <= len(cleaned) <= 4:
        raise SkillError(f"每篇论文必须有 2–4 个论文关键词: {canonical_id}")
    return cleaned
...
def apply_enrichment(state: dict[str, Any], enrichment: dict[str, Any]) -> dict[str, Any]:
    ...
    for paper in state.get("papers", []):
        ...
        paper["keywords"] = validate_keywords(item.get("keywords"), paper)
    return state
...
def validate_state(state: dict[str, Any]) -> dict[str, list[str]]:
    ...
    for paper in state.get("papers", []):
        ...
        try:
            validate_keywords(paper.get("keywords"), paper)
